...
- [How does composition work when a preset's bundle list produces more than one contributor to the same destination file](issues/07-fragment-composition.md) — fragments grouped by target file as `fragments/<target-path>.d/<NN>-<owner>.md`, ordering by filename prefix (no manifest field), owner is a bundle or the preset itself, preset overlay can add `exclude_fragments` to suppress one. Hash-aware anti-clobber from `dotfiles-old` is dropped entirely — nothing persists across launches to protect.
- [How does the dotfiles repo itself get onto a bastion account](issues/08-repo-distribution.md) — repo becomes public (nothing inside is sensitive without its passphrase — identity.age and per-module secrets are both encrypted at rest), fetched via `nix run github:ichsansting/dotfiles` directly with no local git clone, floating on `main` with no version pinning. Carries a constraint into [launch-ux](issues/09-launch-ux.md): the launch command must not require memorizing a preset name upfront.
- [What is the actual shape of the launch command, and does it require flake experimental features pre-enabled](issues/09-launch-ux.md) — bare `nix run github:ichsansting/dotfiles`, no wrapper/alias/args, interactive-only (no `--preset` bypass). Assumes flakes/nix-command are pre-enabled on the target; unmet precondition surfaces as Nix's own error, with the `--extra-experimental-features` fallback documented (not runtime-detected, since Nix refuses before the flake evaluates). Preset selection happens inside the flake's default app via an interactive fzf picker, a separate package surface from the per-preset packages `nix shell` assembles afterward.
- [Sharpen what the preset/bundle editing TUI does and how it relates to the flat bundle/preset model](issues/10-preset-editing-ux.md) — a separate flake app (`nix run .#edit`, Python/Textual, porting `dotfiles-old`'s patterns) run against a real persistent git checkout, not the ephemeral launcher. Full CRUD on bundles/presets, full fragment content+structure editing, secret-item decrypt/edit/re-encrypt (closing a gap `dotfiles-old` left open), and a per-preset materialize-preview dry-run — every edit auto-commits and auto-pushes.
