...
**Revisited 2026-07-14:** hit the "nix-command disabled" error on a real machine, then the documented `--extra-experimental-features` fallback — both behaved exactly as designed. No change to this decision. What actually surfaced was an unrelated bug in `bin/isolate`'s macOS trap cleanup (see ticket 12), fixed separately.
